...
public class SettingsController : BaseController
{
    ...
    /// </summary>
    /// <param name="model">the the change password model</param>
    /// <returns>the result of the change password</returns>
    ...
    public object ChangePassword([FromBody] ChangePasswordModel model)
    {
        ...
        if (string.IsNullOrEmpty(model.Password) || model.Password.Length < 4)
        {
            ...
            {
                success = false,
                message = translator.Instant("Dialogs.ChangePassword.MinimumPasswordError")
            };
        }
        var service = new UserService();
        bool changed = service.ChangePassword(settings.Uid, model.Current, model.Password);
        if (changed)
        {
            return new
            {
                success = true,
                message = translator.Instant("Dialogs.ChangePassword.PasswordChanged")
            };
        }   
        return new
        {
            success = false,
            message = translator.Instant("Dialogs.ChangePassword.PasswordChangeFailed")
        };
    }

    /// <summary>
    /// The model used to change passwords
    /// </summary>
